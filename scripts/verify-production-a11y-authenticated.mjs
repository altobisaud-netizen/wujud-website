#!/usr/bin/env node
/** Production authenticated a11y matrix (ephemeral storage, redacted output). */
...
const apiBase = process.env.PRODUCTION_API_BASE_URL ?? "https://wellness-api-production-production.up.railway.app";
const backendRoot = process.env.WELLNESS_BACKEND_ROOT ?? path.resolve(import.meta.dirname, "..");
const storagePath = path.join(process.env.TEMP ?? process.env.TMP ?? "/tmp", "wujud-prod-auth-storage.json");
...
async function resolveUserId() {
  ...
  const recent = await clerk("/users?limit=5&order_by=-created_at");
...
async function createEphemeralStorageState(browser) {
  ...
  if (!userId) throw new Error("Could not resolve production test user");
  const tokenRes = await clerk("/sign_in_tokens", { method: "POST", body: JSON.stringify({ user_id: userId }) });
  if (!tokenRes?.token) throw new Error("Could not create sign-in token");
  const context = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const page = await context.newPage();
  await page.goto(`${baseUrl}/account/privacy?__clerk_ticket=${encodeURIComponent(tokenRes.token)}`, { waitUntil: "networkidle", timeout: 120000 });
  await page.waitForFunction(() => Boolean(window.Clerk?.session), undefined, { timeout: 120000 });
  ...
  try {
    ...
    throw new Error("Ephemeral auth storage was not created");
  }
}
...
for (const scenario of scenarios) {
  for (const viewport of viewports) {
    ...
    results.push({ scenario: scenario.id, viewport: viewport.label, requiresSignedIn: scenario.requiresSignedIn, signedIn, blockingCount: blocking.length, blockingIds: blocking.map((v) => v.id), mainCount, h1Count, horizontalOverflow: overflow, dir: await page.evaluate(() => document.documentElement.dir) });
    await context.close();
  }
}
await browser.close();
await fs.unlink(storagePath).catch(() => {});
const failed = results.filter((r) => (r.requiresSignedIn && !r.signedIn) || r.blockingCount > 0 || r.horizontalOverflow || r.mainCount !== 1 || r.h1Count < 1);
const report = {
  result: failed.length === 0 ? "FULL_PRODUCTION_AUTHENTICATED_A11Y_PASS" : "REQUIRES_FIX",
  baseUrl,
  apiBase,
  scenariosRun: results.length,
  failedCount: failed.length,
  failures: failed.map(({ scenario, viewport, signedIn, blockingCount, blockingIds, mainCount, h1Count, horizontalOverflow }) => ({ scenario, viewport, signedIn, blockingCount, blockingIds, mainCount, h1Count, horizontalOverflow })),
};
const out = path.resolve(import.meta.dirname, "..", "docs", "wellness-product", "ops-010-production-a11y-authenticated-report.json");
