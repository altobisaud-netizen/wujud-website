...
describe("wellness website structure", () => {
	...
	it("exposes every required direct route", () => {
		...
		expect(index).toContain("Wellness SARA");
		expect(index).not.toMatch(/AI Employees for Modern Businesses|AI sales employee/);
	});

	it("shows launch-list actions after the journey summary", () => {
		...
		const waitlistCta = home.indexOf("t.waitlistCta");
		expect(previewBranch).toBeGreaterThan(-1);
		expect(waitlistCta).toBeGreaterThan(previewBranch);
		expect(locale).toContain("Join the launch list");
		expect(locale).toContain("Nothing from this demo is saved");
	});
	...
	it("has keyboard labels, complete-response live region and reduced motion support", () => {
		...
		expect(read("src/react-app/wellness/locale.ts")).toContain('signIn: "Sign in"');
	...
	it("contains visible safety, privacy, pricing and human-support copy in both languages", () => {
		...
		expect(locale).toContain("Join the launch list");
		expect(locale).toContain("انضم إلى قائمة الإطلاق");
